...
/*    while ( x->x_currpos > x->x_npoints )
	...
	{
		// post("wrapping curr %f against %i", x->x_currpos, x->x_wrap_end );
		x->x_currpos -= x->x_wrap_length;
	}
    while ( x->x_prevpos > x->x_wrap_end )
	{
		// post("wrapping prev %f against %i", x->x_prevpos, x->x_wrap_end );
...
//    maxxindex = x->x_npoints;
	...
	// post("perform doing %i samples", n );
    ...
    while (n){
		...
		// post(" chunk of %i: currgain %f, prevgain %f", chunk, currgain, prevgain );
		...
		for (i = 0; i < chunk; i++){
		    ...
		    /* check indices & wrap */
			float old_xcurrpos = x->x_currpos;
			float old_xprevpos = x->x_prevpos;
	    	d_tabreadmix2_tilde_wrapindices(x);
			/*if ( old_xcurrpos != x->x_currpos || old_xprevpos != x->x_prevpos )
				post("changed: %f->%f, %f->%f, gains %f %f", old_xcurrpos, x->x_currpos, old_xprevpos, x->x_prevpos, currgain, prevgain );
*/
		    /* mix and write */
		    newpos = (int)(*pos++);
			int idx_curr_a = (int)x->x_currpos;
			int idx_curr_b = ((int)x->x_currpos+1)%x->x_wrap_length;
			float idx_factor_curr = x->x_currpos-(int)x->x_currpos;
			int idx_prev_a = (int)x->x_prevpos;
			int idx_prev_b = ((int)x->x_prevpos+1)%x->x_wrap_length;
			float idx_factor_prev = x->x_prevpos-(int)x->x_prevpos;

			/* interpolate */
			float curr = (1.0f-idx_factor_curr)*buf[idx_curr_a] + idx_factor_curr*buf[idx_curr_b];
			float prev = (1.0f-idx_factor_prev)*buf[idx_prev_a] + idx_factor_prev*buf[idx_prev_b];
		    *out++ = currgain * curr + prevgain * prev;
...
//    post( "got input size %f\n", size );
	...
	//post("blocksize set to %f: phase %i, size %i", size, x->x_xfade_phase, x->x_xfade_size );
...
void d_tabreadmix2_tilde_wrap(t_d_tabreadmix2_tilde *x, t_float start, t_float end )
{
    ...
    else
    {
		...
		//post("wrap: %i %i :%i", x->x_wrap_start, x->x_wrap_end, x->x_wrap_length);
    }
}
...
void d_tabreadmix2_tilde_set(t_d_tabreadmix2_tilde *x, t_symbol *s)
{
	...
	//post("d_tabreadmix2_tilde_set: npoints %i xvec %i", x->x_npoints, x->x_vec );
